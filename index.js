...
const authLink = "https://discord.com/api/oauth2/authorize?client_id=945275650477023233&redirect_uri=https%3A%2F%2Ffilelocker.fox3000.repl.co&response_type=code&scope=identify%20guilds"
const app = express();
    ...
    try {
      const oauthResult = await fetch('https://discord.com/api/oauth2/token', {
        ...
        body: new URLSearchParams({
          ...
          redirect_uri: `https://filelocker.fox3000.repl.co`,
...
app.post('/upload', function(req, res) {
  ...
  if (req.files.foo) {
    ...
    let id = uuidv4()
    fileName = `${req.body.id}_${id}.${ext}`
    fileNameWE = `${req.body.id}_${id}`
    ...
    res.redirect('/logged?successful&fileName=' + fileNameWE)
  }
  else
...
async function generatePermissionImage(ids, message, counter = 0) {
  //console.log("Generating pp n°", counter, "for:", ids)
  let coords = coordinates[counter]
  //console.log(coords)
  if (coords) {
    await getPP(ids[counter]).then(async (res) => {
      pp = images(res).size(123)
      let coordsX = coords[0]
      let coordsY = coords[1]
      await canvas.draw(pp, coordsX, coordsY)
      return;
    })
  }
  else {
    let moreImage = images("./public/assets/more.png")
    await canvas.draw(moreImage, 631, 397)
  }
  if (counter == ids.length - 1) {
    ...
    generatePermissionImage(ids, message, counter + 1)
  }
  return;
}
    ...
    for (let i = 0; i < guilds.length; i++) {
      guilds[i].members.fetch().then(r => {
        r.array().sort().forEach(r => {
          ...
          if (username.toLowerCase().indexOf(name.toLowerCase()) != -1 && usersId.indexOf(r.user.id) == -1 && !r.user.bot) {
